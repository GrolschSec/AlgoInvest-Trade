...
class Bruteforce:
    def __init__(self, recursion, file_csv, wallet=500):
        self.data = None
        self.pre_data_treatment(file_csv)
        self.wallet = wallet
        self.best_solution = None
        setrecursionlimit(recursion)
    ...
    def knapsack(self, solution, current_index, wallet):
        """
        ...
        """
        if current_index < 0 or wallet == 0:
            return solution
        # res1 = self.knapsack(solution, currentIndex - 1, wallet)
        # res2 = None
        if self.data.iloc[current_index].loc["Price"] > wallet:
            return self.knapsack(solution, current_index - 1, wallet)
        else:
            res_1 = self.knapsack(solution, current_index - 1, wallet)
            nl = copy(solution)
            nl.append(current_index)
            return self.find_best(
                res_1,
                self.knapsack(
                    nl,
                    current_index - 1,
                    wallet - self.data.iloc[current_index].loc["Price"],
                ),
            )
